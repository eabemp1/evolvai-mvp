...
MODELS = {
    ...
    "ollama-qwen2.5-14b": {
        ...
        "model": "qwen2.5:14b",
        "api_key": None,
        "label": "Ollama Qwen 2.5 14B (Local Free)"
    },
    "ollama-qwen2.5-latest": {
        "provider": "ollama",
        "model": "qwen2.5:latest",
        "api_key": None,
        "label": "Ollama Qwen 2.5 Latest (Local Free)"
    },
    ...
    "ollama-llama3.2-latest": {
        ...
        "api_key": None,
        "label": "Ollama Llama 3.2 Latest (Local Free)"
    }
}
...
def parse_reminder_delete_command(text):
    ...
    if re.match(r"^\s*(clear|delete|remove)\s+(all\s+)?reminders?\s*$", lower):
        return {"mode": "all"}
...
@app.get("/ask")
async def ask(q: str, requester: Optional[str] = None, x_auth_token: Optional[str] = Header(default=None, alias="X-Auth-Token")):
...
@app.get("/debate")
async def debate(q: str, requester: Optional[str] = None, x_auth_token: Optional[str] = Header(default=None, alias="X-Auth-Token")):
...
@app.get("/ask-live")
async def ask_live(q: str, requester: Optional[str] = None, x_auth_token: Optional[str] = Header(default=None, alias="X-Auth-Token")):
